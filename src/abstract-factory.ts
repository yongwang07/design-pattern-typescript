namespace AbstractFactory {
    ...
    class Client<T extends Rocket> {
        constructor(private factory: RocketFactory<T>) {}
        buildRocket() {
            let rocket = this.factory.createRocket();
            rocket.payload = this.factory.createPayload();
            rocket.stages = this.factory.createStages();
            return rocket;
        }
    }
    const r: Rocket = new Client(new FreightRocketFactory()).buildRocket();
}